 ...
 *    node stress-test.js --code 123456 --url http://your-server:3000
 *
 *  OPTIONS:
...
async function joinAllParticipants(url, code, count, batchSize, metrics) {
  const participants = [];
  ...
  for (let batch = 0; batch < totalBatches; batch++) {
    ...
    // Small stagger between batches to not overwhelm the rate limiter
...
async function connectAllSockets(url, participants, metrics, opts) {
  ...
  for (let batch = 0; batch < totalBatches; batch++) {
    ...
    // Stagger batches to avoid SYN flood
    if (batch < totalBatches - 1) {
      await sleep(300);
    }
  }
...
Options:
  ...
  --vote-delay <ms>       Max random delay before voting (default: 0)
  --help                  Show help
    `);
    process.exit(0);
  }
  ...
  console.log(`  Vote Delay:  ${opts.voteDelay}ms`);
  console.log("═══════════════════════════════════════════════════════════");
